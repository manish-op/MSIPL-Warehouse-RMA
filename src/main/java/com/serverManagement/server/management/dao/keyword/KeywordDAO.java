...
public interface KeywordDAO extends JpaRepository<KeywordEntity, Long> {
	...
	public boolean existsByKeyword(@Param("keyword") String keyword);

	@Query("SELECT um FROM KeywordEntity um WHERE LOWER(um.keywordName) = :keyword")
	public KeywordEntity getByKeyword(@Param("keyword") String keyword);

	@Query("SELECT um FROM KeywordEntity um WHERE LOWER(um.keywordName) = :keyword")
	public KeywordEntity getSubKeywordList(@Param("keyword") String keyword);

	// @Query("SELECT CASE WHEN COUNT(um) > 0 THEN TRUE ELSE FALSE END FROM
	// KeywordEntity um WHERE LOWER(um.keywordName) = :keyword AND
	// LOWER(um.subKeyword.subKeyword)= :subKeyword")
