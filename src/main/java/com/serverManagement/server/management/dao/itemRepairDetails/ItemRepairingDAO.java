...
public interface ItemRepairingDAO
		extends JpaRepository<ItemRepairingEntity, Long>, JpaSpecificationExecutor<ItemRepairingEntity> {

	@Query("SELECT um FROM ItemRepairingEntity um WHERE um.id=(:ticketID)")
	public ItemRepairingEntity getRepairingTicketViaTicketId(@Param("ticketID") Long ticketID);

	@Query("SELECT um FROM ItemRepairingEntity um WHERE LOWER(um.serialNo)=(:serialNo)")
	public List<ItemRepairingEntity> getRepairingTicketViaSerialNo(@Param("serialNo") String serialNo);
	...
	public boolean belongingToSameRegion(@Param("regionEntity") RegionEntity regionEntity);
	...
	public TicketDetailsViaIdDTO getRepairingTicketViaTicketIdInDTO(@Param("ticketID") Long ticketID);
}
